// Owner: Blanca Berbel Fernández
// Last edit: 24th February, 2025
...
// CPG X-Y and Z, electrically coupled to X
// Code for exploring a range of g conductance value
...
// Access constants to program parameters

# define PARAMETERS		4  // Number of parameters
# define ENTRY		    1  // File with initial conditions
# define G_FIN			2  // G final value
# define G_PASO			3  // G step
# define TIME			4  // Time simulation


// Number of model's variables
# define DIM			25 		


// Access names to model's variables
...
# define Vm_Y		    8  // Membrane potential of neuron Y
# define Ca_Y		    9
# define mB_Y		    10
# define hB_Y		    11
# define mNaTTX_Y	    12
...
# define Vm_Z		    16  // Membrane potential of neuron Z
# define Ca_Z		    17
# define mB_Z		    18
# define hB_Z		    19
# define mNaTTX_Z	    20
...
// Global variables 
static double V0 = 0.0;  // Initial membrane potential 
static double Ca0 = 0.0;  // Initial calcium concentration 
static double gK_cbm = 0.0;  // all neurons are set in KK parameters for chaotic bursting regime
...
int main (int argc, char ** argv)
{
    ...
    double inct=1e-4;  // time step for simulation
	...
	// Save spikes X and Y
	int stopX = 0, contX = 0, stopY = 0, contY = 0;
	// Save spikes Z
	int stopZ = 0, contZ = 0;

	// Arrays for the model variables
    ...
    if (argc != PARAMETERS + 1)
    {
		...
		puts("Final conductance");
		puts("Step for the conductance");
		puts("Time for simulating");
		exit(0);
    }
	...
	{	
		...
		g_XZ = g_ZX;  // gap junction 

		set_conditions(variables, variables_pre); 

		sprintf(namefileV, "V_gZX_%.4f", g_ZX);
		sprintf(namefileX, "S_X_gZX_%.4f", g_ZX);
		sprintf(namefileY, "S_Y_gZX_%.4f", g_ZX);
		sprintf(namefileZ, "S_Z_gZX_%.4f", g_ZX);
